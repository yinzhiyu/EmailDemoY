...
public class MailEditActivity extends Activity implements OnClickListener {
	...
	private void init(){
		...
		mail_from.setText(BaseApplication.info.getUserName());
		...
		if(mailid>-1){
			Uri uri=Uri.parse("content://com.caogaoxiangprovider");
			Cursor c=getContentResolver().query(uri, null, "mailfrom=? and id=?", new String[]{BaseApplication.info.getUserName(),mailid+""}, null);
			if(c.moveToNext()){
	...
	private void sendMail(){
		BaseApplication.info.setAttachmentInfos(adapter.getList());
		BaseApplication.info.setFromAddress(mail_from.getText().toString().trim());
		BaseApplication.info.setSubject(mail_topic.getText().toString().trim());
		BaseApplication.info.setContent(mail_content.getText().toString().trim());
		//收件人
		...
		BaseApplication.info.setReceivers(recevers);
		...
		new Thread(){
			@Override
			public void run() {
				boolean flag=util.sendTextMail(BaseApplication.info, BaseApplication.session);
